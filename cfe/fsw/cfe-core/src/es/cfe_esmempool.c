...
**
**  $Log: cfe_esmempool.c  $
**  Revision 1.11 2012/01/13 11:50:17EST acudmore 
...
boolean CFE_ES_ValidateHandle(CFE_ES_MemHandle_t  Handle)
{
    ...
    /* Test #1) Handle must be an address on a 32-bit boundary */
    if ((Handle & 0x03) != 0)
    {
        HandleValid = FALSE;
    }
    /* Test #2) Handle must not be a NULL pointer */
    else if (PoolPtr == NULL)
    {
        HandleValid = FALSE;
    }
