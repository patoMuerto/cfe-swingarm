...
*/
    ...
    /* LSW - Read zip file from source folder into local buffer */
    strcpy(Filename, __FILE__);
    pF = strrchr(Filename, '/');
    if (pF != NULL)
    {
      pF++;
      strcpy(pF, "fs_test.gz");
    ...
    /* LSW - Setup to use local buffered data instead of real file I/O */
    UT_SetReadBuffer(Buffer,NumBytes);    
    
    UT_Report(CFE_FS_Decompress("fs_test.gz", "Output") == CFE_SUCCESS ,"CFE_FS_Decompress", "success"); 
    ...
    UT_Report(CFE_FS_Decompress("fake", "Output") == CFE_FS_GZIP_NON_ZIP_FILE ,"CFE_FS_Decompress", "file does not exist"); 
    
    close (fd);
    
    UT_Report(FS_gz_inflate_fixed() == CFE_SUCCESS, "FS_gz_inflate_fixed", "success");
