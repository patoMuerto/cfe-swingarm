...
void Test_SendMsg_MsgLimitExceeded(void){
  ...
  fprintf(UT_logfile, "Begin Test for Msg Limit Exceeded\n");
  SB_ResetUnitTest();
  ...
  ExpRtn = CFE_SUCCESS;
  if(ActRtn != ExpRtn){
    fprintf(UT_logfile, "Unexpected return in Msg Limit Test1,exp=%lx,act=%lx\n",
                        ExpRtn,ActRtn);
    TestStat = CFE_FAIL;
  }
  ...
  ExpRtn = CFE_SUCCESS;
  if(ActRtn != ExpRtn){
    fprintf(UT_logfile, "Unexpected return in Msg Limit Test2,exp=%lx,act=%lx\n",
                        ExpRtn,ActRtn);
    TestStat = CFE_FAIL;
  }
  ...
  CFE_SB_DeletePipe(PipeId);
  SB_ResetUnitTest();
  UT_Report(TestStat, "Test_SendMsg_API","Msg Limit Exceeded Test\n");
